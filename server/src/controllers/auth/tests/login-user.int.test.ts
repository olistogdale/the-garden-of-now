...
describe("POST /login", () => {
  ...
  it("throws an error for invalid email or password input (400)", async () => {
    ...
    const errorReturned = "Invalid login credentials. Please provide a valid email and password."

    await expectError(app, { ...mockLoginInfo, email: 1}, 400, errorReturned);
    await expectError(app, { ...mockLoginInfo, email: "   "}, 400, errorReturned);
    await expectError(app, { ...mockLoginInfo, password: 1}, 400, errorReturned);
    await expectError(app, { ...mockLoginInfo, password: ""}, 400, errorReturned);
    await expectError(app, { ...mockLoginInfo, password: "  TheGardenOfNow "}, 400, errorReturned);
  })
  ...
  it("throws an error for incorrect email or password input (401)", async () => {
    ...
    await expectError(app, { ...mockLoginInfo, email: "jen.smith@email.com"}, 401, errorReturned)
    await expectError(app, { ...mockLoginInfo, password: "TheGardenOfThen" }, 401, errorReturned);
  })
})