...
export const getFavourites = async function (ctx: Context) {
  ...
  if (!Array.isArray(ingredients) || ingredients.length === 0) {
    ...
    ctx.body = {
      error:
        'Invalid ingredients. Please specify a non empty array of ingredients strings.',
    };
    return;
  }
  ...
  const normalizedIngredients = new Set(
    ingredients.filter(Boolean).map(normalize).filter(isNonEmpty),
  );
  ...
  try {
    ...
    const favouriteEntries = Array.isArray(user.favouriteRecipes)
    ...
    const favouriteRecipeMap = new Map<string, RecipeCardT>(
      favouriteRecipes.map((recipe) => [recipe._id, recipe]),
    );

    const recipesOrdered = pageRecipeIds
      .map((id) => favouriteRecipeMap.get(id))
      .filter((recipe): recipe is RecipeCardT => recipe !== undefined);
    ...
    ctx.status = 500;
    ctx.body = {
      error: 'Internal server error: could not fetch favourite recipes.',
    };
  }
};
