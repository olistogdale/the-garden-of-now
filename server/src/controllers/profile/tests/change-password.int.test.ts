...
describe('PATCH /password', () => {
  ...
  it('changes user password for valid authenticated input (204)', async () => {
    ...
    await expect(
      bcrypt.compare('TheGardenOfNow', updatedUser!.passwordHash),
    ).resolves.toBe(false);
    await expect(
      bcrypt.compare('TheGardenOfThen', updatedUser!.passwordHash),
    ).resolves.toBe(true);
  });
  ...
  it('throws an error for invalid password input (400)', async () => {
    ...
    await expectError(
      app,
      { currentPassword: 1, newPassword: 'TheGardenOfThen' },
      400,
      errorReturned,
      accessToken,
    );
    await expectError(
      app,
      { currentPassword: '', newPassword: 'TheGardenOfThen' },
      400,
      errorReturned,
      accessToken,
    );
    await expectError(
      app,
      { currentPassword: '  TheGardenOfNow ', newPassword: 'TheGardenOfThen' },
      400,
      errorReturned,
      accessToken,
    );
    await expectError(
      app,
      { currentPassword: 'TheGardenOfNow', newPassword: 1 },
      400,
      errorReturned,
      accessToken,
    );
    await expectError(
      app,
      { currentPassword: 'TheGardenOfNow', newPassword: 'short' },
      400,
      errorReturned,
      accessToken,
    );
  ...
  it('throws an error for incorrect current password input (401)', async () => {
    ...
    const errorReturned =
      'Invalid password. Please provide a valid current password.';

    await expectError(
      app,
      { currentPassword: 'WrongPassword', newPassword: 'TheGardenOfThen' },
      401,
      errorReturned,
      accessToken,
    );
  ...
  it('throws an error if new password matches current password (400)', async () => {
    ...
    await expectError(
      app,
      { currentPassword: 'TheGardenOfNow', newPassword: 'TheGardenOfNow' },
      400,
      errorReturned,
      accessToken,
    );
