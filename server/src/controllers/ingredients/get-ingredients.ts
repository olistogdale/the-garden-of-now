...
export const getIngredients = async function (ctx: Context) {
  ...
  try {
    const [seasonalIngredientsRetrieval, nonSeasonalIngredientsRetrieval] =
      await Promise.all([
    ...
    const nonSeasonalIngredients = nameAggregator(
      nonSeasonalIngredientsRetrieval,
    );
    const ingredients: string[] = Array.from(
      new Set([...seasonalIngredients, ...nonSeasonalIngredients]),
    );
