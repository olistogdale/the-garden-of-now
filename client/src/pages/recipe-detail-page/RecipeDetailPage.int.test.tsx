...
describe('recipe detail page', () => {
  it('should display a seasonal recipe upon loading', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/irish coddled pork with cider/i),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/host your own st patrick.s day party/i),
      ).toBeInTheDocument();
      expect(
      ...
      expect(mockGetRecipeByID).toHaveBeenCalledWith(
        'irish_pork_1',
  ...
  it('should display a status panel with a recipe failure message if recipe fails to load', async () => {
    ...
    expect(
      await screen.findByText(/couldn.t load recipe: recipe failed to load/i),
    ).toBeInTheDocument();
    expect(
