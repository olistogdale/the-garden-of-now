...
import { mockAuthUser, mockLoginUser, mockGetIngredients } from "../../test/mock-services";
...
describe("login page", () => {
  ...
  it("logs user in and navigates to home page on submission of a valid email and password", async () => {
    ...
    expect(mockLoginUser).toHaveBeenCalledWith({ email: 'bob.simmons@email.com', password: 'TheGardenOfNow' }, expect.any(AbortSignal));
  ...
  it("displays an error message upon submission of an invalid email and password", async () => {
    ...
    const loginErr = new Error("Sorry, this email and password are invalid") as ErrorWithStatusT;
    loginErr.status = 401;
    ...
    expect(mockLoginUser).toHaveBeenCalledWith({ email: 'bob.johnson@email.com', password: 'TheGardenOfThen' }, expect.any(AbortSignal))
    expect(await screen.findByRole("alert")).toHaveTextContent(/sorry, this email and password are invalid/i);
  })
});