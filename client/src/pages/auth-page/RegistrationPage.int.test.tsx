...
import { mockAuthUser, mockRegisterUser, mockGetIngredients } from "../../test/mock-services";
...
describe("registration page", () => {
  ...
  it("enables registration form submission once a valid name, email and password are provided.", async () => {
    ...
    await user.type(screen.getByLabelText(/confirm email/i), "bob.simmons@email.com");
    await user.type(screen.getByLabelText(/^password/i), "TheGardenOfNow");
    await user.type(screen.getByLabelText(/confirm password/i), "TheGardenOfNow");
  ...
  it("registers user and navigates to home page on submission of a valid name, email and password", async () => {
    ...
    expect(mockRegisterUser).toHaveBeenCalledWith({ firstName: "Bob", lastName: "Simmons", email: "bob.simmons@email.com", password: "TheGardenOfNow" }, expect.any(AbortSignal));
  ...
  it("displays an error message upon submission of an invalid name, username or password", async () => {
    ...
    const registerErr = new Error("Sorry, this name, email and password are invalid") as ErrorWithStatusT;
    ...
    await user.type(screen.getByLabelText(/confirm email/i), "pam.simons@email.com");
    await user.type(screen.getByLabelText(/^password/i), "TheGardenOfThen");
    await user.type(screen.getByLabelText(/confirm password/i), "TheGardenOfThen");
    await user.click(screen.getByRole("button", { name: /create account/i }));

    expect(mockRegisterUser).toHaveBeenCalledWith({ firstName: "Pamela", lastName: "Simons", email: "pam.simons@email.com", password: "TheGardenOfThen" }, expect.any(AbortSignal));
    expect(await screen.findByRole("alert")).toHaveTextContent(/sorry, this name, email and password are invalid/i);
  })
})