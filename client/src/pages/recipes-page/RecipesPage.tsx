...
export function RecipesPage({ mode }: Props) {
  ...
  const setParams = useCallback(
    (
      { page: pageNumber, limit: limitValue }: PaginationUpdateT,
  ...
  useEffect(() => {
    ...
    (async function () {
      try {
        ...
        const { recipes, totalCount, totalPages } =
          mode === 'recipes'
            ? await getRecipes(
                { ingredients, seed },
                page,
  ...
  const isPendingIngredients =
    ingredientsStatus === 'idle' || ingredientsStatus === 'loading';
  const showLoadingIngredients = useStableLoading(isPendingIngredients);

  const isPendingRecipes =
  ...
  if (showLoadingIngredients)
    return <StatusPanel mode="loading" message="Loading ingredients…" />;
  ...
  if (showLoadingRecipes)
    return <StatusPanel mode="loading" message="Loading recipes…" />;
