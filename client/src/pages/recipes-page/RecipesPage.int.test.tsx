...
import { mockAuthUser, mockGetIngredients, mockGetRecipes } from "../../test/mock-services";
...
describe("recipes page", () => {
  it("should display seasonal recipes upon loading", async () => {
    ...
    expect(await screen.findByText(/irish coddled pork with cider/i)).toBeInTheDocument();
    expect(await screen.findByText(/gnocchi with roasted squash/i)).toBeInTheDocument();
    expect(mockGetIngredients).toHaveBeenCalledWith(expect.any(AbortSignal));
    expect(mockGetRecipes).toHaveBeenCalledWith(
      expect.objectContaining({
        ingredients: expect.arrayContaining(["butter", "pork", "potato", "cider", "gnocchi", "spinach", "goat's cheese", "butternut squash"]),
  ...
  it("should display a status panel with an ingredients failure message if ingredients fail to load", async () => {
    const ingredientsError = new Error("Ingredients failed to load.") as ErrorWithStatusT;
    ...
    expect(await screen.findByText(/couldn.t load seasonal ingredients: ingredients failed to load/i)).toBeInTheDocument();
  ...
  it("should display a status panel with a recipes failure message if recipes fail to load", async () => {
    ...
    const recipesError = new Error ("Recipes failed to load") as ErrorWithStatusT;
    ...
    expect(await screen.findByText(/well, this is/i)).toBeInTheDocument();
    expect(await screen.findByText(/couldn.t load recipes: recipes failed to load/i)).toBeInTheDocument();
    expect(screen.queryByText(/irish coddled pork with cider/)).not.toBeInTheDocument();
  })
  ...
  it("should paginate recipes across multiple pages if the number of recipes loaded is greater than the limit", async () => {
    ...
    expect(await screen.findByText(/irish coddled pork with cider/i)).toBeInTheDocument();
    expect(screen.queryByText(/gnocchi with roasted squash & goat.s cheese/i)).not.toBeInTheDocument();
    ...
    expect(await screen.findByText(/showing 30 recipes/i)).toBeInTheDocument();
    expect(await screen.findByText(/gnocchi with roasted squash & goat.s cheese/i)).toBeInTheDocument();
    expect(screen.queryByText(/irish coddled pork with cider/i)).not.toBeInTheDocument();
