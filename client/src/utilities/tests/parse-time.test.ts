...
describe('parseTotalDuration', () => {
  ...
  it('returns a structured object that falls back to the sum value of prepTime and cookTime when totalTime input is not present', () => {
    ...
    ).toEqual(structuredOutput2);
    expect(
      parseTotalDuration({
        prepTime: 'PT23H',
        cookTime: 'PT13H10M',
      } as RecipeCardT),
    ).toEqual(structuredOutput3);
    expect(
      parseTotalDuration({
        prepTime: 'PT20M',
        cookTime: 'PT40M',
      } as RecipeCardT),
    ).toEqual(structuredOutput4);
    expect(
      parseTotalDuration({
        prepTime: 'PT20H30M',
        cookTime: 'PT3H30M',
      } as RecipeCardT),
    ).toEqual(structuredOutput5);
    expect(
      parseTotalDuration({
        prepTime: 'PT22H30M',
        cookTime: 'PT3H40M',
      } as RecipeCardT),
    ).toEqual(structuredOutput6);
  });
...
describe('durationToString', () => {
  ...
  it('returns null when input is empty/invalid.', () => {
    ...
    expect(
      durationToString({ daysNum: 0, hoursNum: 0, minsNum: 0 }),
    ).toBeNull();
  });
});
