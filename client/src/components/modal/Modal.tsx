...
export function Modal({ mode, closeModal }: Props) {
  ...
  const [passwordForm, setPasswordForm] = useState<PasswordFormStateT>({
    currentPassword: '',
    newPassword: '',
  });
  ...
  const passwordFormIsInvalid =
    !passwordForm.currentPassword.length || !passwordForm.newPassword.length;
  const canSubmitPasswordForm =
    !passwordFormIsInvalid && !(passwordFormStatus === 'loading');

  const [confirmDeleteStatus, setConfirmDeleteStatus] =
    useState<StatusT>('idle');
  const [confirmDeleteError, setConfirmDeleteError] = useState<string | null>(
    null,
  );
