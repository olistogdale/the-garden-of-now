...
const inputPathRaw = path.join(
  __dirname,
  '../seed-data/clean-data/incomplete-entries-2.json'
);
const rawData = fs.readFileSync(inputPathRaw, 'utf8');
const recipesRaw = JSON.parse(rawData);

const inputPathClean = path.join(
  __dirname,
  '../seed-data/clean-data/incomplete-entries.json'
);
...
let missingRecipes = recipesRaw.filter(
  (recipe) => !cleanRecipeNames.has(recipe.name)
);

const outputPath = path.join(
  __dirname,
  '../seed-data/clean-data/incomplete-entries-3.json'
);
fs.writeFileSync(
  outputPath,
