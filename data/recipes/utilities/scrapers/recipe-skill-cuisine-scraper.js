...
const inputPath = path.join(
  __dirname,
  '../seed-data/clean-data/clean-seed-data-total-updated.json'
);
...
(async function () {
    ...
    if (!recipe.cuisine && !recipe.skillLevel) {
      ...
      if (!recipe.cuisine) {
        ...
        if (schemaHTML) {
          try {
            ...
            console.log(
              `${recipe.name}: Schema does not contain data on cuisine`
            );
          }
        } else {
      ...
      if (!recipe.skillLevel) {
        ...
        if (postContentScriptHTML) {
          try {
            const postContentScript = JSON.parse(
              postContentScriptHTML,
              null,
              2
            );
            recipe.skillLevel =
              postContentScript.skillLevel || recipe.skillLevel;
          } catch {
            console.log(
              `${recipe.name}: Schema does not contain data on skill level`
            );
          }
        } else {
  ...
  const outputPath = path.join(
    __dirname,
    '../seed-data/clean-data/clean-seed-data-total-updated-further.json'
  );
