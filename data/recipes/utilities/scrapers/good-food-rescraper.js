...
const inputPathCleanData = path.join(
  __dirname,
  '../../seed-data/clean-data/clean-seed-data-total.json'
);
const cleanData = JSON.parse(fs.readFileSync(inputPathCleanData, 'utf8'));

const inputPathPartialData = path.join(
  __dirname,
  '../../seed-data/clean-data/entries-missing-mandatory-fields.json'
);
  ...
  for (let entry of parsedDirectoryXml.sitemapindex.sitemap) {
    if (entry.loc[0].includes('recipe')) {
      try {
        ...
        const parsedRecipeDirectoryXml = await parseStringPromise(
          recipeDirectoryResponse.data
        );
          ...
          try {
            ...
            if (schemaHTML && postContentScriptHTML) {
              ...
              try {
                ...
                console.error(
                  `Error parsing schema.org JSON-LD for ${entry.loc[0]}:`,
                  err.message
                );
              }
              try {
                ...
                console.error(
                  `Error parsing postContentScript JSON-LD for ${entry.loc[0]}:`,
                  err.message
                );
              }
            ...
            console.log(
              `Error fetching recipe page and extracting HTML: ${err.message}`
            );
            console.log(
              `Recipe library length is at ${recipes.length} recipes`
            );
          }

          const interval = Math.random() * 1000 + 500;
          console.log(
            `Waiting ${(interval / 1000).toFixed(2)} seconds before next recipe page request...`
          );
      ...
      const outputPath = path.join(
        __dirname,
        `../../seed-data/raw-data/missing-raw-seed-data-${fileID}.json`
      );

      fs.writeFileSync(outputPath, JSON.stringify(recipes, null, 2), 'utf8');
      console.log(
        `Successfully scraped all additional recipes for ${fileID}. Library length: ${recipes.length} recipes`
      );
      ...
      console.log(
        `Waiting ${(interval / 1000).toFixed(2)} seconds before next recipe directory request...`
      );
      await delay(interval);
    }
  }
