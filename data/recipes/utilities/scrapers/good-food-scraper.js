...
let inputPath = path.join(
  __dirname,
  '../seed-data/clean-data/clean-seed-data-total'
)(async function () {
  ...
  for (let entry of parsedDirectoryXml.sitemapindex.sitemap) {
    if (entry.loc[0].includes('recipe')) {
      try {
        ...
        for (let entry of parsedRecipeDirectoryXml.urlset.url) {
          try {
            ...
            console.log(
              `Error fetching recipe page and extracting HTML for ${entry.loc[0]}:`,
              err.message
            );
          }
      ...
      const outputPath = path.join(
        __dirname,
        `../seed-data/seed-data-master-for-${fileID}.json`
      );

      fs.writeFileSync(outputPath, JSON.stringify(recipes, null, 2), 'utf8');
      console.log(
        `Successfully scraped all available recipes for ${fileID}. This recipe library length is at ${recipes.length} recipes`
      );
